...
void Merge_Master() {
    ...
    if (fp1) {
        while (fscanf(fp1, "%d %d %s %s %[^\n]", 
                &temp[count].ward_no, &temp[count].id,
                temp[count].name, temp[count].admission_date,
                temp[count].reason) == 5)
        {
            count++;
        }
    ...
    for (int i = 0; i < count; i++) {
        fprintf(fmaster, "%d %d %s %s %s\n",
                temp[i].ward_no, temp[i].id, temp[i].name,
                temp[i].admission_date, temp[i].reason);
    }
...
void Add_Patient() {
    ...
    Merge_Master();  // ensure master file is updated/sorted
